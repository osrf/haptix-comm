...
int main(int argc, char **argv)
{
  ...
  double initialT2;
  double t2 = 0.0;
  f1.open("validation_1.log");
  f2.open("validation_2.log");
  int counter = 0;
  double error = 0.0;
  ...
  double kMaxError = 30.0;
  double normErr = std::max(0.0, std::min(avgErrorDeg / kMaxError, 1.0));

  std::cout << "Normalized error: " << normErr << std::endl;
