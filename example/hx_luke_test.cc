...
int main(int argc, char **argv)
{
  ...
  // Create the validation log file.
  std::ofstream logFile;
  logFile.open("validation.log");
  ...
  while (motorIndex < 6)
  {
    ...
    while (steps < 250)
    {
      ...
      logFile << sensor.time_stamp.sec << "." << sensor.time_stamp.nsec;

      // Write the state of the actuated joints in the log file.
      logFile <<  " " << sensor.joint_pos[0];
      logFile <<  " " << sensor.joint_pos[2];
      logFile <<  " " << sensor.joint_pos[3];
      logFile <<  " " << sensor.joint_pos[4];
      logFile <<  " " << sensor.joint_pos[5];
      logFile <<  " " << sensor.joint_pos[7];
      logFile << std::endl;
      ...
      unsigned int sleeptime_us = 20000;
#ifdef _WIN32
    ...
    // Wait some time for the first two joints (wrist).
    if (motorIndex < 2)
    {
#ifdef _WIN32
      Sleep((DWORD)(1000));
